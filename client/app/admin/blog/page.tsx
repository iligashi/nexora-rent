        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowEditor(false)}>
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <h2 className="text-lg font-bold text-gray-900">{editingPost ? t.editPost : t.newPost}</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t.excerpt}</label>
                <textarea rows={2} className={inputClass + ' resize-none'} value={form.excerpt} onChange={e => setForm(f => ({ ...f, excerpt: e.target.value }))} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t.content}</label>
              ...
              <div className="flex items-center gap-3">
                ...
                <label htmlFor="published" className="text-sm font-medium text-gray-700">{t.publishImmediately}</label>
              </div>
            </div>
            <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
              <button onClick={() => setShowEditor(false)} className="px-5 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-800">{t.cancel}</button>
              <button onClick={savePost} disabled={saving || !form.title || !form.content} className="px-6 py-2.5 bg-[#FF4D30] text-white text-sm font-semibold rounded-lg hover:bg-[#E6442B] disabled:opacity-50">
                {saving ? t.saving : editingPost ? t.update : t.create}
              </button>
            </div>
          </div>
        </div>
